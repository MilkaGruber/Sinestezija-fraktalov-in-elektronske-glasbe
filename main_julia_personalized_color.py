...
# py -3.11 main_julia.py audio_for_shorts\ZenRacoon-NovoAce_shorter_segment.wav
# FULL: py -3.11 main_julia_personalized_color.py  music/TheCometIsComing-SummonTheFire.wav

# Dependencies:
#   py -3.11 -m pip install numpy glfw moderngl librosa imageio imageio-ffmpeg scipy
# the version where you pass personalized colors
...
class TrackConfig:
    ...
    PAL_RANGE_LOW: list # [HSV colour1:list, HSV colour2,..., HSV colour256] represent the colour palatte to be used
    PAL_RANGE_MID: list
    PAL_RANGE_HIGH: list
...
from color_pal_explorer import LOW_PAL, MID_PAL, HIGH_PAL

TRACK_PRESETS = {
    r"music/TheCometIsComing-SummonTheFire.wav": 
        TrackConfig(par33=0.07, par66=0.075, BEAT_SPEED_INFLUENCE=0.16, STRONG_PULSE_THRESH=0.98, STARTING_ORBITING_SPEED=0.2, SCALING_ORBITING_SPEED=0.05, # cel komad yt
                    SKIP_BEAT=3, BEAT_ZOOM_THRESHOLD=0.20,
                    COOLDOWN_LOW=6, COOLDOWN_MID=4, COOLDOWN_HIGH=4, 
                    # range for colour, range for saturation, range for brightness
                    PAL_RANGE_LOW=LOW_PAL,
                    PAL_RANGE_MID=MID_PAL,
                    PAL_RANGE_HIGH=HIGH_PAL,
                    LOW_DAMPING=0.2, MID_DAMPING=0.4 ,HIGH_DAMPING=1,
                    ),
...
uniform vec3  u_colors[256]; // HSV palette: vec3(h, s, v)
...
// ------------------- palette lookup in HSV -------------------
vec3 paletteLookup(float t){
    int n = 256;
    int idx = int(floor(t * float(n))) % n;
    vec3 hsv = u_colors[idx];
    return hsv2rgb(hsv); // convert to RGB for shading
}
...
vec3 shade(vec4 rr, float seed){  
    ...
    // IF YOU WANT TO DISABLE CIRCLES:
    float detail = 0.16 * sin(0.03 * u_time)* log(trap);
    float k      = fract(1.85*t + bands + detail);

    vec3 fr = paletteLookup(k); 
    fr *= 0.78 + 0.95*edge;

    vec3 glowCol = paletteLookup(fract(k + 0.11));
...
def main():
    ...
    pal = track_config.PAL_RANGE_LOW
    ...
    try:
        for frame in range(total_frames):
            ...
            if pending_pal_change and bi >= pending_pal_start_beat:
                ...
                pal_from = pal.copy()
                if tier_state == 2:
                    pal_to = track_config.PAL_RANGE_HIGH
                if tier_state == 1: 
                    pal_to = track_config.PAL_RANGE_MID
                else:
                    pal_to = track_config.PAL_RANGE_LOW 

                # LOW slower, HIGH faster
                pal_dur_beats = lerp3(10.0, 7.0, 4.0, tier01)

                tier_name = "LOW" if tier_state == 0 else ("MID" if tier_state == 1 else "HIGH")
                print(f"\n[PALLETE CHANGE TRIG] t={t:7.3f}s  beat={bi}  tier={tier_name}")
            ...
            if pal_in_transition:
                ...
                uu = smoothstep(u)
                pal_interp = np.zeros_like(pal_from)

                for i in range(len(pal_from)):
                    for j in range(3):  # h, s, v
                        pal_interp[i, j] = pal_from[i, j] * (1 - uu) + pal_to[i, j] * uu
                if u >= 1.0:
                    pal_in_transition = False
                    pal = pal_to.copy()
            ...
            pal_rgb = [tuple(float(c) for c in color) for color in pal]
            U_fr.set("u_colors", pal_rgb)
